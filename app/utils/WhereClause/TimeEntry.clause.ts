...
class WhereClauseTimeEntry {
  ...
  constructor(bigQ: any, firstQueryObject = {}, user = null) {
    this.base = TimeEntry.find(firstQueryObject);
    this.bigQ = bigQ;
    this.user = user;
  }
  ...
  async search() {
    ...
    if (this.bigQ.search) {
      if (this.bigQ.search.oid) {
        searchword.oid = {
          $regex: this.bigQ.search.oid,
          $options: 'i',
        };
      }
    }
  ...
  totalDocuments() {
    this.totalcountDocument = TimeEntry.estimatedDocumentCount();
    return this;
  }
